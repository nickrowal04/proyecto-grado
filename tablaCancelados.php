...
<link rel="icon" type="image/jpg" href="https://i.ibb.co/BTczYFc/logo-Proyecto.png"/>
	...
	<title>Instrumentos cancelados    </title>
...
<header class="u-clearfix u-gradient u-header u-header" id="sec-a4ee"><div class="u-clearfix u-sheet u-sheet-1">
        ...
        <center><h2 class="u-text u-text-default-lg u-text-default-md u-text-default-sm u-text-default-xl u-text-1"> Instrumentos cancelados </h2></center>
      </div>
</header>
...
<!--
	...
	Tabla de los instrumentos cancelados que mandaron una solicitud de préstamo
       y se cancelo academia de música el Cubarro de Oro
        <br>
		<?php
        include "conexion.php";
        $consltas = "SELECT COUNT(id_prestamo) FROM prestamos where Solicitud cancelada'";
    ...
    <table border="2%" align="center">
		<thead>
			...
			<?php
			include "conexion.php";
			$consltas = "SELECT COUNT(id_prestamo) FROM prestamos where proceso='Solicitud cancelada'";
			$resultado = mysqli_query($conn, $consltas);
		 	while($row = mysqli_fetch_array($resultado)){
				echo "Solicitudes canceladas : $row[0]";
				$nicolas=$row[0];
				if($nicolas==0){
					echo "<br>NO.. hay solicitudes canceladas :D";
				}
			 }
		?>
        </th>
...
//que guarde todo en la variable centencia y que lo guarde en la variable resultado	  
			include "conexion.php";
			$sql = "SELECT * FROM  prestamos where proceso='Solicitud cancelada'" ; 			
