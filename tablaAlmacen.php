...
<link rel="icon" type="image/jpg" href="https://i.ibb.co/BTczYFc/logo-Proyecto.png"/>
	...
	<title>Instrumentos Almacenados    </title>
...
<header class="u-clearfix u-gradient u-header u-header" id="sec-a4ee"><div class="u-clearfix u-sheet u-sheet-1">
        ...
        <center><h2 class="u-text u-text-default-lg u-text-default-md u-text-default-sm u-text-default-xl u-text-1"> Instrumentos almacenados </h2></center>
      </div>
</header>
...
<!--
	...
	Historial de los instrumentos devueltos, están almacenado en la academia  de música el Cubarro de Oro
        <br>
		<?php
        include "conexion.php";
        $consltas = "SELECT COUNT(id_prestamo) FROM prestamos where proceso='Almacenado en la academia'";
        $resultado = mysqli_query($conn, $consltas);
         while($row = mysqli_fetch_array($resultado)){
                echo "El total de los préstamos exitosos son:  $row[0]";
                $nicolas=$row[0];
                    if($nicolas==0){
...
//que guarde todo en la variable centencia y que lo guarde en la variable resultado	  
			include "conexion.php";
			$sql = "SELECT * FROM  prestamos where proceso='Almacenado en la academia'" ; 			
